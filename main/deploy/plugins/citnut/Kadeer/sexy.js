...
module.exports = {
	keywords: ["sexy", "sexxy"],
	name: "sexy",
	description: "plugin này dùng api Sexy của team vangbanlanhat",
	...
	async onCall(message, reply) {
		...
		if (setting.run.boy != true) {
			...
			const res = await axios.get(`https://api.vangbanlanhat.tk/image?type=Sexy`);
			...
			if (r.status == 200) {
				...
				img.path = "Sexy.jpg", img.put(buf), img.stop();
