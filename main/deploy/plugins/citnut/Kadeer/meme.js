...
module.exports = {
	keywords: ["meme"],
	name: "meme",
	description: "plugin này dùng api meme của team vangbanlanhat",
	...
	async onCall(message, reply) {
		...
		if (setting.run.boy != true) {
			...
			const res = await axios.get(`https://api.vangbanlanhat.tk/image?type=Meme`);
			...
			if (r.status == 200) {
				...
				img.path = "Meme.jpg", img.put(buf), img.stop();
