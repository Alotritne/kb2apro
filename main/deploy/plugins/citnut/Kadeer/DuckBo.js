...
module.exports = {
	keywords: ["duckbo", "bo", "ducbo"],
	name: "DuckBo",
	description: "plugin này dùng api DuckBo của team vangbanlanhat",
	...
	async onCall(message, reply) {
		...
		if (setting.run.boy != true) {
			...
			const res = await axios.get(`https://api.vangbanlanhat.tk/image?type=DuckBo`);
			...
			if (r.status == 200) {
				...
				img.path = "DuckBo.jpg", img.put(buf), img.stop();
